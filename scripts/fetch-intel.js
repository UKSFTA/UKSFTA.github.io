...
async function request(url, headers = {}) {
    return new Promise((resolve, reject) => {
        ...
        https.get(url, options, (res) => {
            ...
            res.on('end', () => {
                if (res.statusCode >= 200 && res.statusCode < 300) { try { resolve(JSON.parse(data)); } catch { reject(new Error("JSON Error")); } } 
...
async function fetchBM(range) {
    if (!config.bmKey) {
        console.warn(`[BM_INTEL] Skipping ${range}: BATTLEMETRICS_API_KEY missing.`);
        return [];
    }
    const d = new Date();
    ...
    console.log(`[BM_INTEL] Fetching ${range} for server ${config.bmId} (Key Length: ${config.bmKey.length})`);
    
    try { 
        const res = await request(url, { 'Authorization': `Bearer ${config.bmKey.trim()}` });
        if (res && res.data) {
            console.log(`[BM_INTEL] Successfully retrieved ${res.data.length} data points for ${range}.`);
            return res.data;
        }
        console.warn(`[BM_INTEL] No data returned for ${range}. Raw response:`, JSON.stringify(res).substring(0, 100));
        return [];
    } catch (e) { 
...
async function fetchUC() {
    ...
    try {
        const campaigns = await request(`https://api.unitcommander.co.uk/community/${config.ucId}/campaigns`, h);
        const standalone = await request(`https://api.unitcommander.co.uk/community/${config.ucId}/events`, h);
        
        // Fetch nested events for each campaign
        const fullCampaigns = await Promise.all(campaigns.map(async (cp) => {
            try {
                const events = await request(`https://api.unitcommander.co.uk/community/${config.ucId}/campaigns/${cp.id}/events`, h);
                return { ...cp, events };
            } catch {
                console.warn(`[UC_INTEL] Failed to fetch events for campaign ${cp.id}`);
                return { ...cp, events: [] };
            }
        }));
...
function compress(data) {
    ...
    const sortedData = [...data].sort((a, b) => new Date(a.attributes.timestamp) - new Date(b.attributes.timestamp));
    ...
    for (let i = 0; i < sortedData.length; i++) {
        ...
        const timeDiff = prev ? (new Date(sortedData[i].attributes.timestamp) - new Date(prev.attributes.timestamp)) : 0;
        
        if (i === 0 || i === sortedData.length - 1 || !isS(sortedData[i], sortedData[i - 1]) || !isS(sortedData[i], sortedData[i + 1]) || timeDiff > 30 * 60000) {
...
async function main() {
    ...
    try {
        ...
        } catch { console.warn("[ARMA_UPLINK] Node offline."); }
        ...
        if (uc) {
            ...
            uc.campaigns.forEach(op => {
                ...
                const campaignDir = path.join(process.cwd(), 'content', 'campaigns', slug);
                ...
                if (op.status === "ACTIVE") detectedStatus = "ACTIVE";
                else if (latestEvent && new Date(latestEvent.date) > thirtyDaysAgo) detectedStatus = "ACTIVE";
...
`;
                // Only write if changed or doesn't exist
                if (!fs.existsSync(filePath) || fs.readFileSync(filePath, 'utf8') !== mdContent) {
                    fs.writeFileSync(filePath, mdContent);
                    console.log(`[UC_INTEL] Page Bundle updated: /campaigns/${slug}/_index.md`);
                }

                // --- NEW: Generate Event Nodes ---
                events.forEach(ev => {
                    const evSlug = ev.name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '');
...
`;
                    if (!fs.existsSync(evIndexPath) || fs.readFileSync(evIndexPath, 'utf8') !== evContent) {
                        fs.writeFileSync(evIndexPath, evContent);
                        console.log(`[UC_INTEL] Event node created: /campaigns/${slug}/${evSlug}/`);
                    }
                });
            });
