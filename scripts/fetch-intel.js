...
const config = {
  bmKey: (process.env.BATTLEMETRICS_API_KEY || env.BATTLEMETRICS_API_KEY || '')
    .replace(/^getEnv\s+["']|["']$/g, '')
    .trim(),
  bmId:
    process.env.BATTLEMETRICS_SERVER_ID ||
    env.BATTLEMETRICS_SERVER_ID ||
    '35392879',
  ucId:
    process.env.UNIT_COMMANDER_COMMUNITY_ID ||
    env.UNIT_COMMANDER_COMMUNITY_ID ||
    '722',
  ucToken: (
    process.env.UNIT_COMMANDER_BOT_TOKEN ||
    env.UNIT_COMMANDER_BOT_TOKEN ||
    ''
  )
    .replace(/^getEnv\s+["']|["']$/g, '')
    .trim(),
  ...
  serverPort: parseInt(
    process.env.ARMA_QUERY_PORT || env.ARMA_QUERY_PORT || '2303',
    10,
  ),
  rconPort: parseInt(
    process.env.ARMA_RCON_PORT || env.ARMA_RCON_PORT || '2302',
    10,
  ),
...
async function fetchRconPlayers() {
  ...
  return new Promise((resolve) => {
    ...
    const login = Buffer.concat([
      ...
      Buffer.from(config.rconPass),
    ]);
    client.send(
      createBePacket(login),
      0,
      createBePacket(login).length,
      config.rconPort,
      config.serverIp,
    );
  });
...
async function queryArmaServer() {
  return new Promise((resolve, reject) => {
    ...
    const send = (c = null) =>
      client.send(
        c ? Buffer.concat([A2S_INFO, c]) : A2S_INFO,
        0,
        c ? A2S_INFO.length + 4 : A2S_INFO.length,
        config.serverPort,
        config.serverIp,
      );
    const timeout = setTimeout(() => {
...
async function fetchBM(range) {
  if (!config.bmKey) {
    console.warn(
      `[BM_INTEL] Skipping ${range}: BATTLEMETRICS_API_KEY missing.`,
    );
    return [];
  }
  const d = new Date();
  ...
  console.log(
    `[BM_INTEL] Fetching ${range} for server ${config.bmId} (Key Length: ${config.bmKey.length})`,
  );

  try {
    const res = await request(url, {
      Authorization: `Bearer ${config.bmKey.trim()}`,
    });
    if (res?.data) {
      console.log(
        `[BM_INTEL] Successfully retrieved ${res.data.length} data points for ${range}.`,
      );
      return res.data;
    }
    console.warn(
      `[BM_INTEL] No data returned for ${range}. Raw response:`,
      JSON.stringify(res).substring(0, 100),
    );
    return [];
  } catch (e) {
    console.error(`[BM_INTEL] Request failed for ${range}:`, e.message);
    return [];
  }
}
...
async function fetchUC() {
  ...
  try {
    const campaigns = await request(
      `https://api.unitcommander.co.uk/community/${config.ucId}/campaigns`,
      h,
    );
    const standalone = await request(
      `https://api.unitcommander.co.uk/community/${config.ucId}/events`,
      h,
    );
      ...
      campaigns.map(async (cp) => {
        try {
          const events = await request(
            `https://api.unitcommander.co.uk/community/${config.ucId}/campaigns/${cp.id}/events`,
            h,
          );
          return { ...cp, events };
        } catch {
          console.warn(
            `[UC_INTEL] Failed to fetch events for campaign ${cp.id}`,
          );
          return { ...cp, events: [] };
        }
      }),
    ...
    return { campaigns: fullCampaigns, standalone };
  } catch {
    return null;
  }
}
...
function compress(data) {
  ...
  // Ensure chronological order for timeDiff calculation
  ...
  for (let i = 0; i < sortedData.length; i++) {
    ...
    const timeDiff = prev
      ? new Date(sortedData[i].attributes.timestamp) -
        new Date(prev.attributes.timestamp)
      : 0;

    if (
      i === 0 ||
      i === sortedData.length - 1 ||
      !isS(sortedData[i], sortedData[i - 1]) ||
      !isS(sortedData[i], sortedData[i + 1]) ||
      timeDiff > 30 * 60000
    ) {
...
function generateCampaignMarkdown(op, status) {
  return `---
...
function processCampaignEvents(events, campaignDir, campaignSlug) {
  events.forEach((ev) => {
    const evSlug = ev.name
      .toLowerCase()
      .replace(/[^a-z0-9]+/g, '-')
      .replace(/(^-|-$)/g, '');
    const evDir = path.join(campaignDir, evSlug);
    if (!fs.existsSync(evDir)) {
      fs.mkdirSync(evDir, { recursive: true });
    }

    const evIndexPath = path.join(evDir, '_index.md');
    const evContent = `---
title: "${ev.name}"
...
${ev.description || 'No additional context recovered for this event node.'}
`;
    if (
      !fs.existsSync(evIndexPath) ||
      fs.readFileSync(evIndexPath, 'utf8') !== evContent
    ) {
      fs.writeFileSync(evIndexPath, evContent);
      console.log(
        `[UC_INTEL] Event node created: /campaigns/${campaignSlug}/${evSlug}/`,
      );
    }
  });
}
...
function processCampaigns(campaigns, contentDir) {
  ...
  campaigns.forEach((op) => {
    ...
    let detectedStatus = 'ARCHIVED';
    if (
      op.status === 'ACTIVE' ||
      (latestEvent && new Date(latestEvent.date) > thirtyDaysAgo)
    ) {
      detectedStatus = 'ACTIVE';
    }

    const mdContent = generateCampaignMarkdown(op, detectedStatus);

    if (
      !fs.existsSync(filePath) ||
      fs.readFileSync(filePath, 'utf8') !== mdContent
    ) {
      fs.writeFileSync(filePath, mdContent);
      console.log(
        `[UC_INTEL] Page Bundle updated: /campaigns/${slug}/_index.md`,
      );
    }

    processCampaignEvents(events, campaignDir, slug);
  });
}
...
async function main() {
  ...
  const contentDir = path.join(process.cwd(), 'content', 'campaigns');
  ...
  try {
    ...
    try {
      ...
      console.warn('[ARMA_UPLINK] Node offline.');
    }
    ...
    console.error('X Sync failed:', e.message);
  }
}
main();
