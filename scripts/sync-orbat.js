...
async function main() {
  ...
  if (!BOT_TOKEN) {
    ...
    console.log(
      '  Please ensure you have set up a bot token in the Discord Developer Portal.',
    );
    process.exit(1);
  }
  ...
  try {
    ...
    const personnel = members.map((m) => {
      ...
      const isIC = memberRoles.some(
        (r) => r.name.includes('IC') && !r.name.includes('2IC'),
      );
    ...
    console.log(
      `✓ Successfully synced ${personnel.length} personnel to data/personnel.json`,
    );
  } catch (err) {
