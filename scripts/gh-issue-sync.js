...
async function syncIssues(findings) {
  ...
  const rawIssues = execSync(
    'gh issue list --json title,number --state open --label "automated-audit"',
    { encoding: 'utf8' },
  );
  ...
  for (const issue of existingIssues) {
    if (!findings.find((f) => issue.title.includes(f.id))) {
      ...
      execSync(
        `gh issue close ${issue.number} --comment "Verification complete. Issue cleared."`,
      );
    }
  }
  ...
  for (const finding of findings) {
    if (!existingIssues.find((i) => i.title.includes(finding.id))) {
      ...
      const createOutput = execSync(
        `gh issue create --title "${finding.title}" --body-file temp_body.md --label "automated-audit" --label "security" --label "${finding.severity}"`,
        { encoding: 'utf8' },
      );
      ...
      try {
        execSync(
          `git branch ${branchName} master && git push origin ${branchName}`,
          { stdio: 'ignore' },
        );
